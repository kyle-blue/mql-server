...
class Socket : public SocketOptions {
   public:
    ...
    Socket() : SocketOptions(zmq_socket(Context().ref(), ZMQ_PAIR)){};
    virtual ~Socket() {
        if (0 != zmq_close(m_ref)) {
            Debug(StringFormat("Failed to close socket 0x%0X", m_ref));
        }
    }
    ...
    //--- send string
    bool send(string msg, bool nowait = false) {
        ZmqMsg m(msg);
        return send(m, nowait);
    }
    //--- send empty
    bool send(bool nowait = false) {
        ZmqMsg m;
        return send(m, nowait);
    }
    ...
    bool sendMore(string msg, bool nowait = false) {
        ZmqMsg m(msg);
        return sendMore(m, nowait);
    }
    bool sendMore(bool nowait = false) {
        ZmqMsg m;
        return sendMore(m, nowait);
    }
